...
# Lazy-loaded Playwright — only imported when ENABLE_PLAYWRIGHT=true.
# This lets the bot start without Playwright installed (e.g. on Render free tier).
...
def _get_async_playwright():
    ...
    if not Config.ENABLE_PLAYWRIGHT:
        raise RuntimeError(
            "Playwright is disabled (ENABLE_PLAYWRIGHT=false). "
            "Set ENABLE_PLAYWRIGHT=true and install Chromium to use browser interception."
        )
    if _playwright_module is None:
        try:
            from playwright.async_api import async_playwright as _ap
            _playwright_module = _ap
