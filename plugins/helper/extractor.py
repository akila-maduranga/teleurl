...
If Playwright is disabled (ENABLE_PLAYWRIGHT=false), only direct-URL detection
and yt-dlp fallback work — browser interception is skipped.
"""
...
async def extract_links(url: str, use_browser: bool = True, timeout: int = 25) -> dict:
    ...
    # If it's already a direct media URL, skip browser interception entirely.
    # This is the only path that works when Playwright is disabled.
    if is_direct:
        browser_results = [{
            "url": url,
        ...
        # (will raise if Playwright is disabled)
        try:
    ...
    if not browser_results:
        # ── Strategy 2: yt-dlp fallback (when browser is unavailable) ───────────
        try:
            from plugins.helper.upload import external_extract_ytdlp, YTDLP_AVAILABLE
            if YTDLP_AVAILABLE:
                ydl_info = await external_extract_ytdlp(url)
                if ydl_info and ydl_info.get("formats"):
                    for fmt in ydl_info["formats"]:
                        browser_results.append({
                            "url": fmt.get("url"),
                            "stream_type": _guess_type_from_url(fmt.get("url", "")),
            ...
            errors.append(f"ytdlp_fallback_error: {e}")
